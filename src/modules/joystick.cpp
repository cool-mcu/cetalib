...
WiFiUDP joystickUdpServer;
static int joystickIsActive = 0;                        // used to signal a joystick update 
JoystickState_t joystickState = JOYSTICK_STATE_SERVER_IS_DOWN; // initial joystick UDP Server state
...
{
  // check if WiFi STA is connected to an AP
  if(!network_is_ready())
  {
...
void joystick_tasks(void)
{
  ...
  if(network_is_ready())
  {
    ...
    {
      case JOYSTICK_STATE_SERVER_IS_UP:
        // if there's joystick data available, read/process a packet
        packetSize = joystickUdpServer.parsePacket();
        if (packetSize)
        {
          ...
          sprintf(joystick_local.clientIP, "%s", joystickUdpServer.remoteIP().toString().c_str());
          joystick_local.clientPort = joystickUdpServer.remotePort();
    
          // read the packet into packetBufffer
          int n = joystickUdpServer.read(packetBuffer, UDP_TX_PACKET_MAX_SIZE);
          packetBuffer[n] = 0;
      ...
      case JOYSTICK_STATE_SERVER_IS_DOWN:
        // attempt to re-initialize and restart UDP Server
        // initialize local joystick data structure
        strcpy(joystick_local.clientIP, "0.0.0.0");
        joystick_local.clientPort = 0;
        for (int i=0; i<8; i++)
        ...
        // Start the UDP Server
        SERIAL_PORT.println();
        SERIAL_PORT.print("[Joystick Manager] UDP server re-started on port ");
        SERIAL_PORT.println(localPort);
        joystickUdpServer.begin(localPort);
        joystickState = JOYSTICK_STATE_SERVER_IS_UP;
        break;
  ...
  {
    // WiFi connection is DOWN, close UDP Server
    if(joystickState == JOYSTICK_STATE_SERVER_IS_UP)
    {
      SERIAL_PORT.println();
      SERIAL_PORT.println("[Joystick Manager] WiFI disconnected! Stopping UDP");
      joystickUdpServer.stop();
      joystickState = JOYSTICK_STATE_SERVER_IS_DOWN;
    }
  }
  
}

bool joystick_is_active(void)
{
  if (joystickIsActive)
  {
    joystickIsActive = 0;
